...
/// Mock GitLab v4: MR metadata, approvals, user; everything else `{}`.
async fn mock_gitlab(base: &str, head: &str) -> String {
        ...
        loop {
            ...
            tokio::spawn(async move {
                ...
                loop {
                    ...
                    let is_mr = first.contains("/merge_requests/9")
                        && !first.contains("/discussions")
                        && !first.contains("/notes")
                        && !first.contains("/approve")
                        && !first.contains("/approvals")
                        && !first.contains("/draft_notes");
                    let body = if is_mr {
                        serde_json::json!({
                            ...
                            "changes": [{"old_path": "f.txt", "new_path": "f.txt",
                                "diff": "@@ -1 +1 @@\n-a\n+b\n"}],
                        })
                    } else if first.contains("/approvals") {
                        serde_json::json!({"approved": true, "approved_by": [{"user": {"username": "boss"}}]})
                    } else if first.contains("/members/") {
                        serde_json::json!({"access_level": 40})
                    } else if first.contains("GET /api/v4/user ") {
                        serde_json::json!({"id": 7, "username": "me"})
                    } else if first.contains("/projects/2") && first.contains("GET") {
                        serde_json::json!({"http_url_to_repo": "http://127.0.0.1:9/unused.git", "web_url": "https://x"})
                    } else if first.contains("GET /api/v4/projects/") {
                        serde_json::json!({"web_url": "https://x"})
                    } else {
                        serde_json::json!({})
                    };
...
async fn mr_open_job_swaps_workspace() {
    ...
    let mock = mock_gitlab(&base, &head).await;
